"""
RedCross (Reductive Coupling) Database API Routes

Provides REST endpoints for searching and browsing the RedCross research
database (ligand electronic properties + reductive coupling literature).

Mirrors the pattern of ``routes/database.py`` but scoped to RedCross data.
"""
...
redcross_database_bp = Blueprint(
    'redcross_database', __name__, url_prefix='/api/redcross-database'
)

try:
    from modules.redcross_database import get_redcross_database, RedCrossDatabase
    from modules.redcross_rag import get_redcross_rag, RedCrossRAG
    REDCROSS_DATABASE_AVAILABLE = True
except ImportError:
    REDCROSS_DATABASE_AVAILABLE = False
    logger.warning(
        "RedCross database modules not available. "
        "RedCross Database API endpoints disabled."
    )

try:
    from modules.redcross_similarity import get_similarity_engine
    REDCROSS_SIMILARITY_AVAILABLE = True
except ImportError:
    REDCROSS_SIMILARITY_AVAILABLE = False


def _require_db():
    """Raise 503 if RedCross database module is not available."""
    if not REDCROSS_DATABASE_AVAILABLE:
        raise APIError("RedCross database module not available", 503)


@redcross_database_bp.route('/status')
def redcross_database_status():
    """Get RedCross database status and statistics."""
    _require_db()
    ...
    try:
        ...
        if not db._loaded:
            ...
            if not loaded:
                return jsonify({
                    'success': False,
                    'error': 'Failed to load RedCross database',
                    'available': False,
                }), 503
        ...
        logger.error(f"RedCross database status error: {e}")
        return jsonify({
            'success': False, 'error': str(e), 'available': False,
        }), 500


@redcross_database_bp.route('/ligand-classes')
def ligand_classes():
...
@redcross_database_bp.route('/search/ligands')
def search_ligands():
...
@redcross_database_bp.route('/ligands/<path:name>')
...
@redcross_database_bp.route('/ligands/by-class/<ligand_class>')
...
@redcross_database_bp.route('/compare-classes/<class_a>/<class_b>')
...
@redcross_database_bp.route('/search/reactions')
...
@redcross_database_bp.route('/reactions/<path:doi>')
...
@redcross_database_bp.route('/reactions/by-class/<ligand_class>')
...
@redcross_database_bp.route('/context')
def get_rag_context():
...
@redcross_database_bp.route('/statistics')
def redcross_statistics():
    """Get RedCross database statistics."""
    _require_db()
    ...
    try:
        ...
        logger.error(f"RedCross statistics error: {e}")
        raise APIError(f"Error: {str(e)}", 500)
...
@redcross_database_bp.route('/similarity/<ligand_name>')
def find_similar_ligands(ligand_name: str):
    ...
    """
    if not REDCROSS_SIMILARITY_AVAILABLE:
...
@redcross_database_bp.route('/embedding/data')
@redcross_database_bp.route('/pca/data')  # backward-compatible
...
@redcross_database_bp.route('/pca/loadings')
def pca_loadings():
...
@redcross_database_bp.route('/clusters')
def get_clusters():
...
@redcross_database_bp.route('/clusters/<int:cluster_id>/members')
...
@redcross_database_bp.route('/similarity/method-info')
