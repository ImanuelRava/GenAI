"""
RedCross Nitrogen-Ligand Similarity Engine
...
This module is imported lazily by redcross_rag.py so that the heavy
numpy/sklearn/umap import only fires when the RedCross RAG service is
actually used.
"""
...
def get_similarity_engine() -> LigandSimilarityEngine:
    """Get or create the global similarity engine, fitting on RedCross database."""
    ...
    if not db._loaded:
        if not db.load():
            logger.error("Cannot fit similarity engine: RedCross database not loaded")
