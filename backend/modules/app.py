...
def api_knowledge_graph_upload():
    ...
    try:
        ...
        try:
            ...
            graph_data = generate_kg_from_content(text_content, filename)
...
If a question is outside the scope of redox-active ligands or transition metal catalysis, politely redirect to relevant chemistry topics."""
        ...
        if api_key and provider:
            ...
            try:
                ...
                llm_used = True
                print(f"[Redox Chat] Used user-provided {provider} with model {llm.model}")
            except Exception as e:
        ...
        if response:
            ...
            return jsonify({
                'success': True,
                'response': get_redox_fallback_response(user_message),
                'references': [],
                'llm_used': False,
                'offline_mode': True
            })
