...
from llm.prompts import NICOBOT_SYSTEM_PROMPT, REDCROSS_SYSTEM_PROMPT, EXPLAIN_SYSTEM_PROMPT, PREDEFINED_EXPLANATIONS
...
def _is_ral_rag_available():
    ...
    if _ral_rag_available is None:
        try:
            from modules.redcross_rag import get_ral_rag
            _ral_rag_available = True
        except ImportError:
            _ral_rag_available = False
            logger.warning("REDCROSS RAG service not available.")
...
def build_redcross_system_prompt(message: str, use_rag: bool,
                                log_prefix: str = "REDCROSS-Bot") -> tuple:
    ...
    if _is_ral_rag_available() and use_rag:
        try:
            from modules.redcross_rag import get_ral_rag
            rag = get_ral_rag()
            enhanced = rag.build_enhanced_prompt(message, REDCROSS_SYSTEM_PROMPT)
            # Check if RAG actually added data (enhanced will be longer than base)
            if len(enhanced) > len(REDCROSS_SYSTEM_PROMPT) + 50:
    ...
    return REDCROSS_SYSTEM_PROMPT, None
