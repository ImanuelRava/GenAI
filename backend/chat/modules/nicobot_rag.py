...
class NiCOBotRAG:
    ...
    COMPOUND_KEYWORDS = [
        ...
        'boron', 'magnesium', 'zinc', 'tin', 'silane', 'pivalate',
        'carboxylate', 'epoxide', 'carbamate', 'ether', 'phenol',
        'suzuki', 'heck', 'kumada', 'negishi', 'stille', 'hiyama',
    ]
    ...
    REACTION_KEYWORDS = [
        ...
        'aryl', 'alkylation', 'arylation', 'vinylation', 'reductive',
        'nickel-catalyzed', 'palladium-catalyzed',
    ]
    ...
    MECHANISM_KEYWORDS = [
        ...
        'stereospecific', 'stereoselective', 'enantioselective',
    ]
    ...
    def retrieve_context(self, query: str, max_results: int = 5) -> RAGContext:
        ...
        # --- Search papers (now enriched with abstracts, journal, etc.) ---
        papers = self.db.search_papers(query, limit=max_results)

        # --- Search reaction entries (from Grand_Database) ---
        reaction_entries = self.db.search_reactions(query, limit=max_results)

        # --- Match reaction types by name ---
        query_lower = query.lower()
        for rxn in self.db.reactions.values():
            if rxn.name.lower() in query_lower:
                reactions.append({'id': rxn.reaction_id, 'name': rxn.name})

        # --- General info for mechanism/reaction queries ---
        ...
        formatted = self._format_context(
            compounds, papers, reactions, reaction_entries, general_info
        )
    ...
    def _format_context(
        ...
        # --- Reaction entries (most specific/valuable data) ---
        if reaction_entries:
            parts.append("\n### Specific Reactions from Database:")
            for r in reaction_entries[:5]:
                rxn_name = r.get('reaction_name', 'N/A')
                e_lvg = r.get('e_lvg', 'N/A')
                nu_lvg = r.get('nu_lvg', 'N/A')
                e_major = r.get('e_major', '')
                nu_major = r.get('nu_major', '')
                base = r.get('base', '')
                base_class = r.get('base_class', '')
                e_type = r.get('e_type', '')
                nu_type = r.get('nu_type', '')

                line = f"- **{rxn_name}**"
                if e_major:
                    line += f" | Electrophile: {e_major}"
                if e_type:
                    line += f" ({e_type})"
                if e_lvg and e_lvg != 'None':
                    line += f", LVG: {e_lvg}"
                if nu_major:
                    line += f" | Nucleophile: {nu_major}"
                if nu_type:
                    line += f" ({nu_type})"
                if nu_lvg and nu_lvg != 'None':
                    line += f", Type: {nu_lvg}"
                ...
                if base_class and base_class != 'None':
                    line += f" ({base_class})"
                parts.append(line)
                # Include SMILES for the first 3 reactions
                if reaction_entries.index(r) < 3:
                    r1 = r.get('r1_smiles', '')
                    r2 = r.get('r2_smiles', '')
                    if r1 and r1 != 'None':
                        parts.append(f"  - Electrophile SMILES: `{r1}`")
                    if r2 and r2 != 'None':
                        parts.append(f"  - Nucleophile SMILES: `{r2}`")
        ...
        # --- Papers (with abstracts when available) ---
        if papers:
            ...
            for p in papers[:5]:
                ...
                if p.get('strength'):
                    parts.append(f"  - C-O Bond Strength: {p['strength']}")
                if p.get('electrophile_lvg'):
                    parts.append(f"  - Electrophile LVG: {p['electrophile_lvg']}")
                if p.get('abstract'):
...
When answering, reference specific compounds, papers, or data from the database when relevant. If the user asks about a specific compound or reaction, provide the SMILES notation and any relevant publication references from the database."""
    ...
    def get_database_stats_response(self) -> str:
        ...
        if 'reaction_entries' in stats:
            response += f"- **{stats['reaction_entries']}** specific reaction entries\n\n"
        else:
            response += "\n"
